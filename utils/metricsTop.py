...
class MetricsTop():
    ...
    def __eval_mosei_regression(self, y_pred, y_true, exclude_zero=False):
        ...
        #计算y_pred当中有多少非0值
        non_zeros = np.array([i for i, e in enumerate(y_pred) if e != 0])
        print(f'non_zeros {non_zeros.shape}')
        print(f'non_zeros number {len(non_zeros)}')
        print(f'zeros {len(y_pred) - len(non_zeros)}')
        
        #计算y_true当中有多少非0值
        non_zeros = np.array([i for i, e in enumerate(y_true) if e != 0])
        print(f'non_zeros {non_zeros.shape}')
        print(f'non_zeros number {len(non_zeros)}')
        print(f'zeros {len(y_true) - len(non_zeros)}')
        
        test_preds = y_pred.view(-1).cpu().detach().numpy()
        test_truth = y_true.view(-1).cpu().detach().numpy()
        print(f'test_preds {test_preds.shape}')
        print(f'test_truth {test_truth.shape}')
        print(f'type(test_preds) {type(test_preds)}')
        print(f'type(test_truth) {type(test_truth)}')
